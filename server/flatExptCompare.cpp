...
FlatExptCompare::FlatExptCompare(uint* gs, uint gsSze, vector<probe_set*>* data, uint* expts, uint exptN, float sgma, float ordr, set<void*>* processes, QMutex* mutex){
  ...
  cout << "FlatExptCompare set some stuff " << endl;
...
FlatExptCompare::~FlatExptCompare(){
...
void FlatExptCompare::normalise(float** v, uint s1, uint s2){
...
void FlatExptCompare::mean(float** v, float* m, uint s1, uint s2){
  ...
  //float order = 5;   // u
  // the above defined in the constructor and set by the user.. 
  
  // M_E is e.. 
  //cout << endl;
  // -- various versions used before.. 
  // current version flattens low values at the bottom and high values at the top between values of 0-1 by transposing to a 
  // a sigmoidal curve.. -- after setting the minimal value to 0...  -- ehh, how to expalin this.. 
  for(int i=0; i < s2; i++){
...
float** FlatExptCompare::growValueArray(float** v, uint oldSize, uint newSize){
...
// 	}
  ...
  for(uint i=0; i < exptNo; i++){
    for(uint j= (i+1); j < exptNo; j++){
      //distances[i][j] = sqrt(distances[i][j]);   // this is a bit hmmm, -- but squared distances give us nicer patterns.. which is sort of understanbale.. 
